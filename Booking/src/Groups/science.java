	    ...
	    String member = (String)request.getAttribute("member");
	     ...
	     if(member.equals("2members")) {
	    	  ...
	    	  for(int i=0;i<reg2.length;i++)	{
	    		  ...
	    		  if((stu.getFaculty().equals("ENGINEERING"))) {
	    			  int one = gd.removegroup((String)session.getAttribute("regno"));
	    			request.setAttribute("failuregrps", "Group members should not be from ENGINEERING faculty ");
					RequestDispatcher rd = request.getRequestDispatcher("Creategroup.jsp");
					rd.forward(request, response);
				
	    		  }
	    		  if((stu.getFaculty().equals("HEALTH SCIENCE"))) {
	    			  int one = gd.removegroup((String)session.getAttribute("regno"));
	    			request.setAttribute("failuregrps", "Group members should not be from HEALTH SCIENCE faculty ");
					RequestDispatcher rd = request.getRequestDispatcher("Creategroup.jsp");
					rd.forward(request, response);
